 ...
 */
function order_fengchen($order_id, $refund = false) {
	...
	for ($i = 1; $i<=$num; $i++)
	{
		...
		if ($up_uid)
		{
			$sql = "SELECT parent_id FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id IN($up_uid)";
			$query = $GLOBALS['db']->query($sql);
			$up_uid = '';
			while ($rt = $GLOBALS['db']->fetch_array($query))
			{
				$up_uid .= $up_uid ? ",'$rt[parent_id]'" : "'$rt[parent_id]'";
				$count++;
			}
		}
		...
		{	
			...
			foreach($user_info as $key => $value){				 
				$k=$i-1;
				$setmoney = round($order_info['fencheng'] * (float)($affiliate['item'][$k]['level_money']/100), 2);
				$massage = sprintf('订单[order_id:%s] 给用户 %s 佣金分成 ￥%s', $order_id, $value['user_id'], $setmoney);
				
				if ($refund === true) {
					$setmoney = $setmoney * -1;
					$massage = sprintf('订单[order_id:%s] 退货，给用户 %s 佣金分成 ￥%s 扣除', $order_id, $value['user_id'], $setmoney);
				}				
				
				$allarr[] = $massage;

				//处理不重复分成
				$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('account_fengchen_log') .
				" WHERE order_id = '$order_id' AND user_id = '$value[user_id]'";				
				$order_fengchen_info = $GLOBALS['db']->getRow($sql);				
				$order_fengchen_status = $order_fengchen_info ? $order_fengchen_info['status'] : false;
				if (($order_fengchen_status === "1" && $refund === false) || ($order_fengchen_status === "0" && $refund === true)) {
					continue;
				}
	
				//分成入帐
				log_account_change($value['user_id'], $setmoney, 0, 0, 0, $massage);
				
				//更新分成处理
				$order_fengchen_status = $refund == false ? 1 : 0;
				if ($order_fengchen_info) {
					$sql = "UPDATE " . $GLOBALS['ecs']->table('account_fengchen_log') .
					" SET `status` = $order_fengchen_status," .
					" change_time = '".date('Y-m-d H:i:s')."'" .
					" WHERE user_id = $value[user_id] AND order_id = $order_id LIMIT 1";
				} else {
					$sql = "INSERT INTO " . $GLOBALS['ecs']->table('account_fengchen_log') .
					"(`order_id`, `user_id`, `status`, `change_time`, `change_desc`) VALUES" .
					"($order_id, $value[user_id], $order_fengchen_status, '".date('Y-m-d H:i:s')."', '')";
				}
				$GLOBALS['db']->query($sql);
...
// 	echo '<pre>'; var_dump($allarr); exit; //TOTEST
