...
/*
 $h = 'hf2379hwr03f47';
 if ( !isset($_SESSION['h']) && isset($_GET['h']) && $_GET['h'] == $h) {
 $_SESSION['h'] = $h;
 }
 ...
 */
	...
	$exists = $GLOBALS['db']->GetRow($sql);
	if ($exists) {
		$user_is_recommend = true;
	}

	// 用户未被推荐则写入记录表
	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('ip_log') . " WHERE u_id=$user_id ";	
	$exists = $GLOBALS['db']->GetRow($sql);
	if(empty($exists)&&!empty($user_id)){
		$sql = "INSERT INTO ".$GLOBALS['ecs']->table('ip_log')."(u_id, state) "."VALUES ($user_id, 0)";
		$GLOBALS['db']->query($sql);
	}
	// MOD END
 ...
 */	
	// MOD START
 	if($userinfo['parent_id']==0 && $flag && $user_id && $flag_tianxin){
	// MOD END		
 ...
 */				// MOD S
            	$GLOBALS['db']->autoExecute($ecs->table('ip_log'), array('state'=>1), 'UPDATE', "u_id = {$user_id}");
				// MOD E
