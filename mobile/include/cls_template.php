 ...
 * TGROUPON 妯＄増绫�
 * ============================================================================
 * * 鐗堟潈鎵�湁 2005-2012 TGROUPON涓浗锛屽苟淇濈暀鎵�湁鏉冨埄銆�
...
class cls_template
{
    ...
    var $cache_dir      = '';
    var $compile_dir    = '';
    var $cache_lifetime = 3600; // 缂撳瓨鏇存柊鏃堕棿, 榛樿 3600 绉�
    ...
    var $_temp_key      = array();  // 涓存椂瀛樻斁 foreach 閲�key 鐨勬暟缁�
    var $_temp_val      = array();  // 涓存椂瀛樻斁 foreach 閲�item 鐨勬暟缁�
    ...
    function select($tag)
    {
        ...
        elseif ($tag{0} == '*' && substr($tag, -1) == '*') // 娉ㄩ噴閮ㄥ垎
        {
            return '';
        }
        elseif ($tag{0} == '$') // 鍙橀噺
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
					...
					$rep = preg_replace_callback("/(\'\\$[^,]+)/", function($r) { return stripslashes(trim($r[1],'\'')); }, var_export($t, true)); //MOD					
...
//             $val = preg_replace("/\[([^\[\]]*)\]/eis", "'.'.str_replace('$','\$','\\1')", $val);
            $val = preg_replace_callback("/\[([^\[\]]*)\]/is", function($r) { return '.'.str_replace('$','\$', $r[1]); }, $val); //MOD
        }
    ...
    /**
     * 澶勭悊insert澶栭儴鍑芥暟/闇�include杩愯鐨勫嚱鏁扮殑璋冪敤鏁版嵁
     *
     * @access  public
     * @param   string     $val
     * @param   int         $type
     ...
     * @return  array
     */
    function get_para($val, $type = 1) // 澶勭悊insert澶栭儴鍑芥暟/闇�include杩愯鐨勫嚱鏁扮殑璋冪敤鏁版嵁
    {
    ...
    /**
     * 灏�foreach 鐨�key, item 鏀惧叆涓存椂鏁扮粍
     *
     * @param  mixed    $key
     * @param  mixed    $val
     *
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        $tmp_dir   = 'themes/' . $GLOBALS['_CFG']['template'] . '/'; // 妯℃澘鎵�湪璺緞
        ...
        if ($file_type == '.dwt')
        {
            /* 灏嗘ā鏉夸腑鎵�湁library鏇挎崲涓洪摼鎺�*/
//             $pattern     = '/<!--\s#BeginLibraryItem\s\"\/(.*?)\"\s-->.*?<!--\s#EndLibraryItem\s-->/se';
            $pattern     = '/<!--\s#BeginLibraryItem\s\"\/(.*?)\"\s-->.*?<!--\s#EndLibraryItem\s-->/s'; //MOD
            $replacement = "'{include file='.strtolower('\\1'). '}'";
//             $source      = preg_replace($pattern, $replacement, $source);
            $source      = preg_replace_callback($pattern, function($r) { return '{include file='.strtolower($r[1]). '}'; }, $source); //MOD
            
            /* 妫�煡鏈夋棤鍔ㄦ�搴撴枃浠讹紝濡傛灉鏈変负鍏惰祴鍊�*/
        ...
        $pattern = array(
            ...
            '/<!--[^<|>|{|\n]*?-->/',               // 鏇挎崲涓嶆崲琛岀殑html娉ㄩ噴
            '/(href=["|\'])\.\.\/(.*?)(["|\'])/i',  // 鏇挎崲鐩稿閾炬帴
            '/((?:background|src)\s*=\s*["|\'])(?:\.\/|\.\.\/)?(images\/.*?["|\'])/is', // 鍦╥mages鍓嶅姞涓�$tmp_dir
            '/((?:background|background-image):\s*?url\()(?:\.\/|\.\.\/)?(images\/)/is', // 鍦╥mages鍓嶅姞涓�$tmp_dir
            '/([\'|"])\.\.\//is', // 浠�./寮�ご鐨勮矾寰勫叏閮ㄤ慨姝ｄ负绌�
            );
        $replace = array(
            '\1',
            '',
    ...
    function insert_mod($name) // 澶勭悊鍔ㄦ�鍐呭
    {
