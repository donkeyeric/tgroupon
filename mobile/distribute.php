    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
			...
			foreach($user_info as $key=>$value){
				/*TGROUPON分销微分销开发未付款的佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=0 and order_status in (".OS_CONFIRMED . ',' . OS_UNCONFIRMED .")";
				...
				/*TGROUPON分销微分销开发已经付款且未收货佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and  pay_status=2 and   shipping_status  <> 2 and order_status in (".OS_CONFIRMED . ',' . OS_UNCONFIRMED .")";
				$order_info=$db->getRow($sql);
				$k=$i-1;
				...
				/*TGROUPON分销微分销开发已经收货佣金统计*/
				$sql="SELECT count(*) as order_num ,sum(fencheng)  as order_amount FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']."  and  shipping_status  =2 and order_status in (".OS_SPLITED . ',' . OS_SPLITING_PART .")";
				$order_info=$db->getRow($sql);
				$k=$i-1;
...
elseif ($action == 'act_edit_surplus')
{
    ...
    //如果订单已确认，佣金分成给用户
    if ($order_status_confirmed === true) {
    	order_fengchen($order_id);
    }
